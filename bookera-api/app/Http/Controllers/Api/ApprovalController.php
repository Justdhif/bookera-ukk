...
class ApprovalController extends Controller
{
     ...
     */
    public function approveReturn(Request $request, BookReturn $bookReturn)
    {
        if ($bookReturn->approval_status !== 'pending') {
            return ApiResponse::errorResponse(
        ...
        $bookReturn = DB::transaction(function () use ($bookReturn) {
            ...
            foreach ($bookReturn->details as $detail) {
                ...
                $newStatus = $detail->condition === 'lost' ? 'lost' : 'available';
            ...
            ActivityLogger::log(
                'update',
                'book_return',
                "Book return #{$bookReturn->id} approved by admin",
                [
        ...
        return ApiResponse::successResponse(
            'Pengembalian buku berhasil disetujui',
            $bookReturn
        );
    }

    /**
     * Reject book return request
     */
    public function rejectReturn(Request $request, BookReturn $bookReturn)
    {
        if ($bookReturn->approval_status !== 'pending') {
            return ApiResponse::errorResponse(
                'Pengembalian ini sudah diproses sebelumnya',
        ...
        ActivityLogger::log(
            'update',
            'book_return',
            "Book return #{$bookReturn->id} rejected by admin",
            [
                'return_id' => $bookReturn->id,
                'approval_status' => 'rejected',
                'reason' => $data['rejection_reason'] ?? 'No reason provided',
    ...
    /**
     * Get all pending return approvals
     */
    public function getPendingReturns(Request $request)
    {
        ...
        return ApiResponse::successResponse(
            'Data pengembalian yang menunggu persetujuan',
            $returns
        );
    }

...
export function AdminBadgeIcon({ className }: { className?: string }) {
  return (
    <Badge
      variant="outline"
      className={cn(
...
export default function AdminBadge({ className, showIcon = true }: AdminBadgeProps) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn(
        "relative overflow-hidden group px-2 py-0.5 flex items-center gap-1.5 shrink-0 border-brand-primary/20 dark:border-brand-primary/30 bg-brand-primary/5 dark:bg-brand-primary/10 backdrop-blur-sm text-brand-primary transition-all duration-300 hover:shadow-lg hover:shadow-brand-primary/30 h-5",
        className
      )}
    >
      <div className="absolute inset-0 bg-linear-to-r from-brand-primary/0 via-brand-primary/10 to-brand-primary/0 -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-in-out" />

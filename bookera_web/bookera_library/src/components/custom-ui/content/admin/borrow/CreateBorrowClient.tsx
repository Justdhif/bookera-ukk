...
export default function CreateBorrowClient() {
  ...
  const fetchBooks = async (page = 1) => {
    try {
      ...
      const response = await bookService.getAll({ per_page: ITEMS_PER_PAGE_OPTIONS[1], page });
  ...
  const fetchUsers = async (page = 1) => {
    try {
      ...
      const response = await userService.getAll({ per_page: ITEMS_PER_PAGE_OPTIONS[1], page });
      const meta = response.data.data;

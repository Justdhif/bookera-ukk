          ...
          <div className="container mx-auto px-6 md:px-12 lg:px-20 relative z-10">
            <div className="flex flex-col lg:flex-row items-center gap-16">
              <div className="flex-1 space-y-8">
                <Badge className="bg-brand-primary/20 text-brand-primary border-brand-primary/30 px-3 py-1">
                  <Bot className="w-3.5 h-3.5 mr-2" /> {t("aiBadge")}
                </Badge>
                <div>
                  <h2 className="text-4xl md:text-5xl font-bold mb-4">{t("aiTitle")}</h2>
                  <p className="text-xl text-gray-400 font-medium">{t("aiSubtitle")}</p>
                </div>
                ...
                <div className="space-y-6 pt-4">
                  {[
                    { title: t("aiFeature1"), desc: t("aiFeature1Desc"), icon: Sparkles },
                    { title: t("aiFeature2"), desc: t("aiFeature2Desc"), icon: BookOpen },
                    { title: t("aiFeature3"), desc: t("aiFeature3Desc"), icon: MessageCircle }
                  ].map((feat, i) => (
              ...
              <div className="flex-1 w-full max-w-md lg:max-w-none">
                <div className="relative rounded-3xl bg-gray-900 border border-gray-800 shadow-2xl p-6 overflow-hidden">
                  <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-brand-primary via-teal-400 to-brand-primary" />
                  
                  <div className="flex items-center gap-4 mb-6 pb-4 border-b border-gray-800">
                    <div className="w-12 h-12 rounded-full bg-brand-primary/20 flex items-center justify-center p-2">
                      <Image src={BoteraImg} alt="Botera AI" width={40} height={40} className="object-contain drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                    </div>
                    <div>
                      <h4 className="font-semibold text-white">{t("aiBotera")}</h4>
                      <p className="text-xs text-brand-primary flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-brand-primary animate-pulse" /> {t("aiOnline")}</p>
                    </div>
                  </div>

                  <div ref={chatScrollRef} className="space-y-4 mb-6 h-64 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-transparent">
                    <div className="bg-gray-800 rounded-2xl rounded-tl-sm p-4 text-sm max-w-[90%] whitespace-pre-wrap text-gray-300 mr-auto">
                      {t("aiWelcome")}
                    </div>
                    {chatHistory.map((chat, idx) => (
                      <div key={idx} className={cn(
                        "rounded-2xl p-4 text-sm max-w-[90%] whitespace-pre-wrap",
                        chat.role === "bot" 
                          ? "bg-gray-800 rounded-tl-sm text-gray-300 mr-auto" 
                          : "bg-brand-primary rounded-tr-sm text-white ml-auto"
                      )}>
                        {chat.text}
                      </div>
                    ))}
                    {isTyping && (
                      <div className="bg-gray-800 rounded-2xl rounded-tl-sm p-4 w-16 text-sm text-gray-300 mr-auto flex gap-1 justify-center items-center h-10">
                        <span className="w-1.5 h-1.5 rounded-full bg-gray-500 animate-bounce" />
                        <span className="w-1.5 h-1.5 rounded-full bg-gray-500 animate-bounce delay-150" />
                        <span className="w-1.5 h-1.5 rounded-full bg-gray-500 animate-bounce delay-300" />
                      </div>
                    )}
                  </div>

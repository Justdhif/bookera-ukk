...
import { FadeIn, FadeUp, BlurIn, ScaleIn } from "@/components/custom-ui/motion";
...
export default function ChatDetailSheet({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if ((!newMessage.trim() && selectedFiles.length === 0) || !activeUser || isModerating) return;
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="p-0 w-full sm:max-w-lg border-l-0 flex flex-col h-full bg-background shadow-2xl gap-0">
        <SheetHeader className="p-4 pr-12 flex-row items-center gap-3 bg-card/95 backdrop-blur-sm shrink-0 h-17 border-b space-y-0 text-left">
          <Link 
          ...
          >
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2 min-w-0">
                <SheetTitle className="font-semibold text-sm leading-tight truncate group-hover:text-brand-primary transition-colors">
              ...
              <p className="text-xs text-muted-foreground truncate">
                @{activeUser.profile?.username || activeUser.slug || t("userFallback")}
              </p>
            </div>
          </Link>
        </SheetHeader>
        ...
        {moderationAlert && (
          <FadeIn className="px-4 py-3 bg-linear-to-r from-red-500/10 via-orange-500/10 to-red-500/10 border-b border-red-200/50 dark:border-red-800/30 shrink-0">
          ...
          <ScrollArea className="h-full relative z-10">
            ...
            <div className="flex flex-col gap-4 p-4 min-h-full">
              {loading ? (
                ...
                <>
                  <FadeUp className="flex justify-center mb-0">
                    <div className="bg-amber-100 dark:bg-[#1a1608] border border-amber-200 dark:border-amber-900 px-6 py-2 rounded-xl max-[85%] shadow-md">
                  ...
                  {messages.length === 0 ? (
                    <div className="space-y-6">
                      <FadeIn className="flex items-center justify-center py-4">
                      ...
                      <FadeUp delay={0.1} className="flex flex-col items-center justify-center py-10 opacity-70">
                    ...
                    groupedMessages.map((group) => (
                      <div key={group.date} className="flex flex-col space-y-4">
                        ...
                        {group.messages.map((msg, i) => {
                          const decryptedText = msg.message && currentUser && activeUser 
                            ? decryptMessage(msg.message, currentUser.id, activeUser.id)
                            : msg.message;
                                ...
                                >
                                  ...
                                  {msg.image_path && (
                                    <div className={cn(
                                      ...
                                      Array.isArray(msg.image_path) && msg.image_path.length > 1 ? "grid grid-cols-2" : "flex"
                                    )}>
            ...
            <div className="flex gap-3 overflow-x-auto pt-2 pr-2 pb-2 scrollbar-hide">
              {imagePreviews.map((url, index) => (
                <div key={url} className="relative inline-block shrink-0">
                  <img src={url} alt={`Preview ${index}`} className="h-20 w-20 rounded-md border border-border shadow-sm object-cover" />
                  <Button variant="destructive" size="icon" className="absolute -top-2 -right-2 h-5 w-5 rounded-full shadow-md" onClick={() => handleRemoveImage(index)}>
                    <X className="h-3 w-3" />
                  </Button>
                </div>
              ))}
        ...
        <div className="p-4 bg-card/95 backdrop-blur-sm z-10 shrink-0">
          ...
          >
            <Button type="button" variant="ghost" size="icon" onClick={() => fileInputRef.current?.click()} className="rounded-full shrink-0 text-muted-foreground hover:text-foreground h-10 w-10">
            ...
            <Button
              ...
              disabled={(!newMessage.trim() && selectedFiles.length === 0) || loading || isModerating}
            >

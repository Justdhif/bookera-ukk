        ...
        {loading ? (
          ...
          <StaggerContainer className="space-y-4">
            {reviews.data.map((r, idx) => {
              const isMe = currentUser?.id === r.user?.id;
              const profileHref = isMe ? "/my-profile" : `/${r.user?.slug}`;
              return (
                <FadeUp key={r.id} delay={idx * 0.05}>
                  <div className="flex gap-4 p-4 border rounded-xl bg-card hover:bg-muted/30 transition-colors">
                    <Link
                      href={profileHref}

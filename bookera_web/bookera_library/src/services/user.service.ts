...
export const userService = {
  ...
  requestPhoneChange: (newPhone: string) =>
    api.post<ApiResponse<{ phone_hint: string }>>("/phone/request-change", {
      new_phone: newPhone,
    }),

  verifyPhoneOtp: (otp: string) =>
    api.post<ApiResponse<User>>("/phone/verify-otp", { otp }),

  requestEmailChange: (newEmail: string) =>
    api.post<ApiResponse<{ email_hint: string }>>("/email/request-change", {
      new_email: newEmail,
    }),

  verifyEmailOtp: (otp: string) =>
    api.post<ApiResponse<User>>("/email/verify-otp", { otp }),
};

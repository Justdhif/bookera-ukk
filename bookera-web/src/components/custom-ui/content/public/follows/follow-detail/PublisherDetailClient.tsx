...
import { ArrowLeft, Building2, BookOpen, Heart, HeartOff, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { followService } from "@/services/follow.service";
import { PublisherDetail } from "@/types/follow";
import { useAuthStore } from "@/store/auth.store";
import FollowDetailBookList from "./FollowDetailBookList";

export default function PublisherDetailClient() {
  ...
  const fetchPublisher = async () => {
    try {
      const res = await followService.getPublisherDetail(slug);
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      if (publisher.is_following) {
        await followService.unfollow("publisher", publisher.id);
        toast.success(`Unfollowed ${publisher.name}`);
        setPublisher({ ...publisher, is_following: false });
        window.dispatchEvent(new Event("refreshFollowsList"));
        router.push("/");
      } else {
        await followService.follow("publisher", publisher.id);
        toast.success(`Now following ${publisher.name}`);
        setPublisher({ ...publisher, is_following: true });
        window.dispatchEvent(new Event("refreshFollowsList"));
      }
    } catch {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-6 max-w-7xl">
      <div className="space-y-4 mb-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex flex-col justify-end flex-1">
            <p className="text-muted-foreground text-sm">Publisher</p>
            ...
            <div className="lg:mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <span className="font-medium">
                {publisher.books_count} {publisher.books_count === 1 ? "book" : "books"}
              </span>
            </div>
            <div className="mt-3 flex gap-2">
              {isAuthenticated && (
                <Button
                  variant={publisher.is_following ? "outline" : "brand"}
                  size="sm"
      ...
      <FollowDetailBookList books={publisher.books || []} />
    </div>
  );
}

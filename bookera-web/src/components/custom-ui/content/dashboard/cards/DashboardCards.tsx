  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      {items.map((item) => (
        <Card
          key={item.label}
          className={`group relative overflow-hidden border ${item.borderColor} ${item.hoverBorderColor} transition-all duration-300 hover:shadow-lg dark:hover:shadow-slate-900/30 bg-gradient-to-br ${item.bgGradient} backdrop-blur-sm cursor-pointer`}
          onClick={() => handleDetailsClick(item.detailsHref)}
        >
          {/* Animated gradient border effect */}
          <div className="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-transparent group-hover:via-white/10 dark:group-hover:via-white/5 transition-all duration-500 pointer-events-none" />

          {/* Corner accent matching sidebar design */}
          <div
            className={`absolute top-0 right-0 w-16 h-16 bg-gradient-to-br ${item.gradient} opacity-5 dark:opacity-10 rounded-bl-full transition-opacity group-hover:opacity-10 dark:group-hover:opacity-15`}
          />

          {/* Pulsing dot indicator */}
          <div
            className={`absolute top-3 right-3 h-2 w-2 rounded-full bg-gradient-to-br ${item.gradient} animate-pulse ring-2 ring-background`}
          />
            ...
            <div className="flex flex-col">
              <div className="flex items-end justify-between">
                <div className="flex items-baseline gap-2">
                  <p className="text-3xl font-bold tracking-tight bg-gradient-to-br from-slate-900 to-slate-700 dark:from-slate-100 dark:to-slate-300 bg-clip-text text-transparent">
                    {item.value.toLocaleString()}
                  </p>
                  <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-gradient-to-r from-white/50 to-white/30 dark:from-slate-800/50 dark:to-slate-900/50 backdrop-blur-sm border border-slate-200/30 dark:border-slate-700/30">
                    <BarChart3 className="h-3 w-3 text-slate-600 dark:text-slate-400" />
                    <span className="text-[10px] font-bold uppercase tracking-wider bg-gradient-to-br from-slate-700 to-slate-900 dark:from-slate-300 dark:to-slate-100 bg-clip-text text-transparent">
                      Stats
                    </span>
                  </div>
                </div>
              ...
              <div className="mt-4 relative">
                <div className="absolute left-0 right-0 h-px bg-gradient-to-r from-transparent via-slate-300/30 dark:via-slate-600/30 to-transparent" />
                <div
                  className={`relative h-px bg-gradient-to-r ${item.gradient} opacity-60 transition-all duration-700 group-hover:opacity-80`}
                  style={{ width: "60%" }}
                />
              </div>
              ...
              <div className="mt-3 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <div
                    className={`h-2 w-2 rounded-full bg-gradient-to-br ${item.gradient} animate-pulse`}
                  />
          ...
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
            <div
              className={`absolute inset-0 bg-gradient-to-br ${item.gradient} opacity-[0.03] dark:opacity-[0.05]`}
            />
          </div>

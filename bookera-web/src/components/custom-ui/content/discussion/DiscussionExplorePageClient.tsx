  ...
  const fetchBrowsePage = useCallback(
    async (pageNum: number, append = false) => {
      ...
      try {
        ...
        const res = await discussionPostService.getPosts({
          page: pageNum,
          per_page: PER_PAGE,
        });
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {posts.map((post) => {
              ...
              const avatarSrc = post.user?.profile?.avatar_url
                ? post.user.profile.avatar_url
                : post.user?.profile?.avatar
                  ? post.user.profile.avatar.startsWith("http")
                    ? post.user.profile.avatar
                    : `${process.env.NEXT_PUBLIC_STORAGE_URL}/${post.user.profile.avatar}`
                  : undefined;

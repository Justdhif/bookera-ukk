...
function CommentItem({ comment, postSlug, postOwnerId, onDelete, onReply }: CommentItemProps) {
  ...
  const handleDeleteReply = async (id: number) => {
    try {
      await discussionCommentService.deleteComment(id);
      ...
      <div className="flex gap-3 group">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1 ml-1">
            ...
            <button
              className="text-[11px] font-semibold text-muted-foreground hover:text-purple-600 transition-colors"
          ...
          {hasReplies && visibleReplies.length > 0 && (
            <div className="mt-2 space-y-2 pl-2 border-l-2 border-purple-100 dark:border-purple-900">
                  ...
                  <div key={reply.id} className="flex gap-2 group/reply">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-0.5 ml-1">
                        ...
                        <button
                          className="text-[10px] font-semibold text-muted-foreground hover:text-purple-600 transition-colors"
                          onClick={() =>
                            onReply?.(
                              reply.id,
                        ...
                        {user?.id === reply.user_id && (
                          <button
                            className="text-[10px] text-muted-foreground hover:text-destructive opacity-0 group-hover/reply:opacity-100 transition-all"
              ...
              {!showAllReplies && remainingReplies > 0 && (
                <button
                  className="mt-1 ml-1 flex items-center gap-1 text-[11px] font-semibold text-purple-600 hover:text-purple-700"
...
const CommentSection = forwardRef<CommentSectionHandle, CommentSectionProps>(
  ...
  const loadComments = async () => {
    ...
    setOpen(true);
    try {
      const res = await discussionCommentService.getComments(postSlug);
      setComments(res.data.data.data);
      setLoaded(true);
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await discussionCommentService.addComment(postSlug, {
    ...
    <>
      {!defaultOpen && (
        <button
          ...
          className="flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-purple-600 transition-colors"
        >
